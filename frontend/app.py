...
def fetch_projects():
    """Fetch list of all projects from API."""
    try:
...
def fetch_all_assessments():
    ...
    try:
        # This would need a new API endpoint or we can fetch all projects and their assessments
        # For now, we'll fetch all projects and get their assessment dates
        projects = fetch_projects()
        all_dates = set()
        
        for project in projects:
            assessments = fetch_project_assessments(project['project_id'])
            dates = get_available_dates(assessments)
            all_dates.update(dates)
        
        return sorted(list(all_dates), reverse=True)
    except requests.exceptions.RequestException as e:
        st.error(f"Error fetching assessment dates: {e}")
        return []

def fetch_latest_assessments(project_id):
    """Fetch latest risk assessments for a project."""
    try:
        response = requests.get(f"{API_BASE_URL}/api/projects/{project_id}/latest")
        response.raise_for_status()
        return response.json()
    except requests.exceptions.RequestException as e:
        st.error(f"Error fetching latest assessments: {e}")
        return []

def fetch_project_assessments(project_id):
    """Fetch all historical assessments for a project."""
    try:
        response = requests.get(f"{API_BASE_URL}/api/projects/{project_id}/assessments?limit=1000")
        response.raise_for_status()
        return response.json()
    except requests.exceptions.RequestException as e:
        st.error(f"Error fetching project assessments: {e}")
        return []
...
def fetch_assessments_by_date(assessment_date):
    ...
    try:
        ...
        # For now, we'll fetch all projects and filter by date
        projects = fetch_projects()
        date_assessments = []
        ...
        for project in projects:
            ...
            for assessment in assessments:
                if assessment['rating_date'] == assessment_date:
                    date_assessments.append(assessment)
        
        return date_assessments
    except requests.exceptions.RequestException as e:
        st.error(f"Error fetching assessments by date: {e}")
        return []
...
def fetch_projects_with_assessment_date(assessment_date):
    ...
    try:
        ...
        st.error(f"Error fetching projects with assessment date: {e}")
        return []
